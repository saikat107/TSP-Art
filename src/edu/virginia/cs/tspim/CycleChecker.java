        ...
        for (int sourcevertex = 0; sourcevertex < number_of_nodes; sourcevertex++){
         ...
         while (!stack.isEmpty()){
             ...
             while (i < number_of_nodes){
                 if (adjacencyMatrix[element][i] >= 1.0 && visited[i] == 1){
                     if (stack.contains(i)){
                         cyclepresent = true;
                         return cyclepresent;
                     }
                 }
                 if (adjacencyMatrix[element][i] >= 1.0 && visited[i] == 0){
                     stack.push(i);
                     visited[i] = 1;
                     adjacencyMatrix[element][i] = 0.0;// mark as labelled;
                     adjacencyMatrix[i][element] = 0.0;
                     element = i;
                     i = 1;
                     continue;
                  }
